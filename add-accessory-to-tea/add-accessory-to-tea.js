$(document).ready(function () {
    console.log("ready - add accessory to tea screen");
    ...
    const url = "http://localhost:8080/teatime/accessory/allExceptContainingTea/" + teaId;
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            divCard.onclick = function(){
                ...
                if(divCard.getAttribute("selected") === "true"){
                    divCard.setAttribute("selected", "false");
                    divCard.className = "card";
                    let index = selectedAccessories.indexOf(divId);
                ...
                selectedAccessories.push(divId);
            ...
            let addSelectedButton = document.getElementById("add-selected-button");
            addSelectedButton.onclick = function () {
                ...
                for (let i = 0; i < selectedAccessories.length; i++) {
                    ...
                    let url = "http://127.0.0.1:8080/teatime/tea/accessory/add/" + teaId + "/" + selectedAccessories[i];
                    console.log("add acc to tea url: " + url);
                    $.ajax({
                        url: url,
                        type: 'PUT',
                        data: data,
                        success: function(data) {
                            alert('Accessory added to tea!');
                            location.reload();
                        },
                        error: function () {
